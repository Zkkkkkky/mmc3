...
class UnitWeaponCodec:
    ...
    def record_offset(self, unit_id: int) -> int:
        ...
        return self.rom.profile.unit_weapon_table_offset + unit_id * UNIT_WEAPON_SLOT_COUNT

    def _validate_table(self, data: bytes) -> None:
        profile = self.rom.profile
        end = profile.unit_weapon_table_offset + profile.unit_count * UNIT_WEAPON_SLOT_COUNT
        if end > len(data):
            raise RomFormatError("机体武器配置表超出 ROM。")
        for unit_id in range(1, profile.unit_count):
            start = profile.unit_weapon_table_offset + unit_id * UNIT_WEAPON_SLOT_COUNT
    ...
    def decode(self, unit_id: int, data: bytes | None = None) -> UnitWeaponConfig:
