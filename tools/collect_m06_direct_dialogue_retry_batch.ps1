...
$logPath = Join-Path $repoRoot "output\verification\m06-direct-dialogue-retry-batch-20260920.log"
$configs = Get-ChildItem -LiteralPath (Join-Path $repoRoot "tools\golden_pipeline\cases") -Filter "M06-dialogue-*-cold-start-01.json" |
  Where-Object {
    $_.Name -like "*-segment-*" -or
    ($_.Name -like "*defense*-number-*")
  } |
  Sort-Object Name
Set-Location -LiteralPath $repoRoot
"M06 direct dialogue retry: $($configs.Count) cases" | Set-Content -LiteralPath $logPath -Encoding utf8
