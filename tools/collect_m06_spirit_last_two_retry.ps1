...
$logPath = Join-Path $repoRoot "output\verification\m06-spirit-last-two-retry-20260920.log"
$cases = @(
    "M06-spirit-cost-08-cold-start-01.json",
    "M06-spirit-cost-11-cold-start-01.json"
)
Set-Location -LiteralPath $repoRoot
"M06 spirit last two retry started: $(Get-Date -Format o)" | Set-Content -LiteralPath $logPath -Encoding utf8
foreach ($name in $cases) {
    ...
    $stdout = Join-Path $repoRoot "output\verification\$tag-retry4-stdout-20260920.log"
    $stderr = Join-Path $repoRoot "output\verification\$tag-retry4-stderr-20260920.log"
...
"`nM06 spirit last two retry finished: $(Get-Date -Format o)" | Add-Content -LiteralPath $logPath -Encoding utf8
