...
$logPath = Join-Path $repoRoot "output\verification\m06-spirit-final-retry-20260920.log"
$cases = @(
...
"M06 spirit final retry started: $(Get-Date -Format o)" | Set-Content -LiteralPath $logPath -Encoding utf8
foreach ($name in $cases) {
    ...
    $stdout = Join-Path $repoRoot "output\verification\$tag-retry3-stdout-20260920.log"
    $stderr = Join-Path $repoRoot "output\verification\$tag-retry3-stderr-20260920.log"
...
"`nM06 spirit final retry finished: $(Get-Date -Format o)" | Add-Content -LiteralPath $logPath -Encoding utf8
