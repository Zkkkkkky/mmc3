"""Enumerate the owner-drawn M04 page, context menu and shop editor dialog."""
...
OUT = ROOT / "output/verification/legacy-m04-reference-controls-20260920"
...
def main() -> int:
    ...
    session = ui.ProbeSession(ROOT, "m04-reference-controls-20260920")
    session.out = out
    ...
    try:
        ui.stage_a(session)
        if not session.open_rom_via_menu(probe_rom, "M04_打开ROM对话框"):
            raise RuntimeError("M04 probe ROM did not open")
        ...
        if len(maps) != 1:
            raise RuntimeError(f"M04 map control 500 matched {len(maps)}")
        map_control = maps[0]
        ...
        if not ui.real_click_at(session.main_hwnd, map_x, map_y, button="right"):
            raise RuntimeError("M04 map right click failed")
        time.sleep(0.4)
        ui.capture_screen_region(
            ui.get_window_rect(session.main_hwnd),
            session.shots / "M04_商店事件_右键菜单.png",
            pad=80,
        )
        ...
        if not dialog:
            raise RuntimeError("M04 shop editor dialog did not open")
        session.dump_window(dialog["hwnd"], "M04_商店设置", menu=False)
        payload = json.loads(
            (session.controls / "M04_商店设置.json").read_text(encoding="utf-8")
        )
        visible = [
            item
