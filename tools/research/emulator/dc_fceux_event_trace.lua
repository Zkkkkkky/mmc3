...
local output = assert(io.open("../../../output/verification/fceux/dc_fceux_event_trace.log", "w"))
local frame = 0
local hits = 0
local seen = {}
local bank_select = 0
...
for current = 1, 12000 do
    ...
    if current == 1000 or current == 2000 or current == 11990 then
        gui.savescreenshotas(string.format("../../../output/verification/fceux/dc_event_frame_%05d.png", current))
    end
end
