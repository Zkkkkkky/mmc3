local out=[[D:\GIT\mmc3\output\verification\v14-purple-]]
local log=assert(io.open([[D:\GIT\mmc3\output\verification\v14-purple-state.tsv]],"w"))
log:write("frame\tcrossing\tr0\ttop_r1\tsplit0_r1\tsplit1_r1\tr5_0\tr5_1\n")
...
for frame=1,7400 do
  ...
  if frame>=6750 then
    ...
    log:write(string.format("%d\t%d\t%02X\t%02X\t%02X\t%02X\t%02X\t%02X\n",frame,crossing,memory.readbyte(0x04D8),memory.readbyte(0x04D9),memory.readbyte(0x03C6),memory.readbyte(0x03C7),memory.readbyte(0x03EE),memory.readbyte(0x03EF)))
  end
