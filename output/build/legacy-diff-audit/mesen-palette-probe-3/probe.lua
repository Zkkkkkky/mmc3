...
emu.addEventCallback(function()
    ...
    -- Start a new game, then advance its introductory prompts at a human-like pace.
    local press_a = (frame >= 690 and frame < 696) or
                    (frame >= 850 and frame % 90 < 3)
    local press_start = frame >= 780 and frame < 786
    local ok, message = pcall(
        emu.setInput, 0, {a = press_a, start = press_start}, false)
    if not ok then
    ...
    elseif input_polls <= 4 or frame == 690 or frame == 850 then
        audit(string.format(
            "INPUT frame=%d poll=%d requested_a=%s requested_start=%s",
            frame, input_polls, tostring(press_a), tostring(press_start)))
    end
...
emu.addEventCallback(function()
    ...
    if frame == 675 or frame == 750 or frame == 1000 or frame == 1500 or
       frame == 2200 or frame == 3200 or frame == 5000 or frame == 8000 then
        snapshot("checkpoint")
    end
    if frame == 8000 then
        print(string.format(
