...
emu.addEventCallback(function()
    ...
    if input_polls <= 4 or frame == 690 or frame == 850 then
        local state = emu.getInput(0)
        print(string.format(
            "INPUT frame=%d poll=%d requested_a=%s requested_start=%s actual_a=%s actual_start=%s",
            frame, input_polls, tostring(press_a), tostring(press_start),
            tostring(state.a), tostring(state.start)))
    end
