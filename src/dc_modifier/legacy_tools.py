...
class MapAnimationDialog(QDialog):
    """Faithful navigation shell for the unverified map-animation tables."""
    ...
    REFERENCE_INSTRUCTIONS = (
        "切换00区域的图库号：图库编号 0A",
        "调用颜色（物理）：颜色三字节 28 17 20",
        "等待：001帧",
        "创建物体：物体编号 0F，X坐标 248，Y坐标 248",
        "调取物体运行规律：参考规律 [01]001",
        "调用音乐：音乐代码 02",
        "等待：029帧",
        "切换00区域的图库号：图库编号 08",
        "调用颜色（物理）：颜色三字节 30 23 0F",
        "动画结束",
        "空代码",
        "空代码",
        "空代码",
    ...
    def __init__(self, parent: QWidget | None = None, project: Any | None = None) -> None:
        ...
        self.tabs.addTab(self._readonly_tab("动画调用"), "动画调用")
        root.addWidget(self.tabs, 1)
        self.read_only_status = QLabel("只读：右侧为参考界面指令，尚未从当前ROM解析。")
        self.read_only_status.setMaximumHeight(22)
    ...
    def _animation_tab(self) -> QWidget:
        ...
        left_group = QGroupBox("动画选择")
        left_group.setFixedWidth(365)
        left = QVBoxLayout(left_group)
        self.animation_list = QListWidget()
        self.animation_list.addItems(self.ANIMATION_NAMES)
        self.animation_list.setCurrentRow(0)
        self.animation_list.currentRowChanged.connect(self._refresh_animation_inspector)
        left.addWidget(self.animation_list, 1)
        self.add_button = QPushButton("添加")
        self.add_button.setEnabled(False)
        self.add_button.setToolTip(READ_ONLY_NOTICE)
        left.addWidget(self.add_button)
        left.addWidget(QLabel("动画名称:"))
        ...
        right_group = QGroupBox("动画编辑")
        right = QVBoxLayout(right_group)
        self.instruction_table = QTableWidget(0, 1)
        self.instruction_table.verticalHeader().hide()
        self.instruction_table.horizontalHeader().hide()
        ...
        right.addWidget(self.instruction_table, 1)
        self.code_button = QPushButton("代码编辑")
        self.code_button.setEnabled(False)
        self.code_button.setToolTip(READ_ONLY_NOTICE)
        right.addWidget(self.code_button, 0, Qt.AlignmentFlag.AlignLeft)
        layout.addWidget(right_group, 3)
        self._refresh_animation_inspector(0)
        return page

    def _readonly_tab(self, label: str) -> QWidget:
        page = QWidget()
        layout = QVBoxLayout(page)
        notice = QLabel(f"{label}表的指针、长度和指令语义尚未完成差分验证。")
        notice.setWordWrap(True)
        layout.addWidget(notice)
        inspector = QPlainTextEdit()
        inspector.setReadOnly(True)
        inspector.setPlainText("只读保护：当前版本不定位、不改写未知ROM区域。")
        layout.addWidget(inspector, 1)
        return page

    def _refresh_animation_inspector(self, row: int) -> None:
        if row < 0:
            return
        label = self.ANIMATION_NAMES[row]
        self.animation_name.setText(label.split("：", 1)[-1])
        self.instruction_table.setRowCount(len(self.REFERENCE_INSTRUCTIONS))
        for index, instruction in enumerate(self.REFERENCE_INSTRUCTIONS):
            item = QTableWidgetItem(
                f"{index:03d}：[参考界面/未从ROM解析] {instruction}"
            )
            item.setToolTip("此行为参考截图内容，不是从当前ROM解码所得。")
            self.instruction_table.setItem(index, 0, item)
        profile = getattr(getattr(self.project, "profile", None), "key", "未载入ROM")
        digest = getattr(self.project, "source_sha256", "—")
