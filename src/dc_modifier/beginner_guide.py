...
GUIDE_TOPICS = (
    ...
    GuideTopic(
        ...
        (
            ...
            "拼图图库右键可导入/复制/删除；碎片效果图左键改位置，右键切换水平翻转。",
            "点“暂存当前机体”。",
    ...
    GuideTopic(
        ...
        (
            "在左侧选择人物。",
            "勾选精神；要改消耗就双击精神名称。头像图库/位置用下拉选择，颜色直接点色块。",
            "台词按钮会显示实际正文；点击选择。特殊规则先选中一行，再编辑条件或台词，也可新增/删除。",
            "导出头像时选择目录，会生成 [正面].bmp、[背面].bmp、[效果].bmp。",
            "点“应用当前人物修改”，最后点右下角“确定”。",
        ),
    ...
    GuideTopic(
        ...
        (
            "在左侧选择武器。",
            "修改基本属性；敌我动画页右键“编辑”只修改当前指令参数。右键“插入”先只选择新的指令类型，下一步才设置这条新指令的参数，避免把当前指令误当成新增模板。",
            "点“规律…”可按编号或名称查找；代码可复制、导出，也可切换完整记录查看。",
            "点“应用当前武器修改”。需要实战时再点“动画测试”。",
            "数据库窗口点“确定”，主窗口再 Ctrl+S。",
        ),
        "武器属性与安全动画参数写入派生 ROM；动画测试不会覆盖原 ROM 或原存档。",
        "规律代码和未验证指针保持只读；规律名称可保存到 .dcmod 工程，浏览/导出不会改 ROM。",
    ),
    GuideTopic(
        "数据库：战斗对话",
        "database:3",
    ...
    GuideTopic(
        ...
        (
            "在左侧选择关卡。",
            "选择上方页签；已有事件右键“编辑”只修改当前参数，“插入（接上/接下）”才打开三页事件指令选择器。",
