...
ACTION_FIELDS = {
    ...
    0x1F: ("提升数值", "附加标志"),
    0x20: ("HP低字节", "HP高字节"),
    0x21: ("地址低字节", "地址高字节", "位掩码"),
    0x22: ("属性地址低字节", "属性地址高字节", "位掩码"),
    0x23: ("属性地址低字节", "属性地址高字节", "比较值", "附加标志"),
    0x24: ("左上X", "左上Y", "右下X", "右下Y", "机体数量", "阵营/状态"),
    0x25: ("范围", "机体数量"),
    0x26: (),
    0x27: ("选项编号/状态",),
    0x28: (),
    ...
    0x2F: (),
    0x30: ("战场动画ID",),
    0x31: ("HP变化值低字节", "HP变化值高字节"),
    0x32: ("HP变化值低字节", "HP变化值高字节"),
    0x33: ("HP变化值低字节", "HP变化值高字节"),
    0x34: ("恢复百分比",),
    0x35: ("状态地址低字节", "状态位", "设置/取消状态"),
    0x36: ("目标人物ID",),
    0x37: ("扩展参数1", "扩展参数2"),
    0x38: ("目标人物ID",),
    0x39: ("精神恢复值",),
    0x3A: ("左上X", "左上Y", "右下X", "右下Y", "HP变化值低字节", "HP变化值高字节", "阵营/状态"),
    0x3B: ("精神效果ID",),
    0x3C: ("选项数量",),
    0x3D: ("左上X", "左上Y", "右下X", "右下Y", "HP变化值低字节", "HP变化值高字节", "阵营/状态"),
    0x3E: ("范围", "HP变化值低字节", "HP变化值高字节"),
    0x3F: ("范围", "HP变化值低字节", "HP变化值高字节"),
    0x40: ("头像ID", "文本组", "文本编号"),
    ...
    0x56: ("重复地址低字节", "重复地址高字节", "重复/状态"),
    0x57: ("跳转地址低字节", "跳转地址高字节"),
    0x58: ("跳转地址低字节", "跳转地址高字节"),
    0x59: ("音乐命令",),
    0x5A: ("音乐命令",),
    0x5B: ("音乐命令",),
    0x5C: ("属性地址（低字节）", "属性地址（高字节）", "属性值", "置数/增加/减少与偏移"),
    0x5D: ("属性地址（低字节）", "属性地址（高字节）", "属性值（低字节）", "属性值（高字节）", "运算方式", "偏移值", "附加参数"),
    0x5E: ("等待帧数",),
    0x5F: (),
    0x60: ("限制代码",),
    0x61: ("限制代码",),
    ...
    0x6B: (),
    0x6C: (),
    0x6D: ("经验值低字节", "经验值高字节", "金钱低字节", "金钱高字节"),
    0x6E: ("人物ID",),
    0x6F: ("人物ID", "机体ID", "等级", "队伍/成长参数"),
    0x70: ("提升等级",),
    0x71: (),
    0x72: (),
    0x73: (),
    0x74: ("扩展参数",),
    0x75: ("X", "Y", "人物ID", "机体ID", "等级", "AI/标志"),
    0x76: ("X", "Y", "人物ID", "机体ID", "等级", "AI/标志"),
...
OPCODE_HELP = {
    ...
    0x30: "播放一段战场动画；从下拉框选择动画编号。",
    0x31: "修改指定人物的 HP；变化量由低、高字节组成。",
    0x32: "按参考版的第二种 HP 处理方式修改人物 HP。",
    0x33: "按参考版的第三种 HP 处理方式修改人物 HP。",
    0x34: "按百分比恢复当前人物 HP。",
    0x35: "设置或取消指定状态位；状态位和操作方式分别选择。",
    0x36: "忽略通常机动力限制，强制向指定人物移动。",
    0x38: "把当前人物传送到指定人物周围。",
    0x39: "恢复当前人物的精神值。",
    0x3A: "修改指定矩形范围内机体的 HP。",
    0x3B: "对敌方单位应用指定精神效果。",
    0x3C: "建立一个分支选项事件，后续判断读取选择结果。",
    0x3D: "按参考版的另一种方式修改矩形范围内机体 HP。",
    0x3E: "修改当前人物周围机体的 HP。",
    0x3F: "按参考版的另一种方式修改周围机体 HP。",
    ...
    0x5B: "立即播放指定背景音乐。",
    0x5C: "按参考修改器的“属性修改”规则，对指定地址的单字节数值进行置数、增加或减少。",
    0x5D: "按参考修改器的“属性修改”规则，对指定地址的双字节数值进行置数、增加或减少。",
    0x5E: "等待指定帧数后继续事件；60 帧约为 1 秒。",
    0x60: "限制当前人物移动。",
    0x61: "解除当前人物的移动限制。",
    ...
    0x6B: "让当前单位撤退并离开战场。",
    0x6C: "刷新当前行动人物的移动坐标。",
    0x6D: "直接增加经验值和金钱；两项数值都由低、高字节组成。",
    0x6E: "让下拉框中选择的人物离开我方队伍。",
    0x6F: "让选择的人物与机体直接加入我方队伍。",
    0x70: "提升当前队员的等级。",
    0x71: "判定本关胜利。",
    0x72: "判定本关失败。",
    0x73: "进入游戏通关流程。",
