...
from tools import build_dc_expanded_dual_audio as builder
...
class ExpandedDualAudioTests(unittest.TestCase):
    ...
    def test_source_rom_is_unchanged_and_output_is_derived(self) -> None:
        self.assertEqual(
            hashlib.sha256(builder.SOURCE_ROM.read_bytes()).hexdigest(),
            builder.EXPECTED_SOURCE_SHA256,
        )
        self.assertNotEqual(builder.SOURCE_ROM, builder.OUTPUT_ROM)
    ...
    def test_three_music_banks_and_engine_match_payloads(self) -> None:
        for music_bank in builder.MUSIC_BANK_ASM:
            payload = (
                builder.ASM_OUTPUT_ROOT / f"dc_dual_music_{music_bank:02x}.bin"
            ).read_bytes()
