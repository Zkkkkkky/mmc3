...
> **零经验用户入口：**随时按 `F1` 或选择“帮助 → 新手操作向导”。向导不要求理解 ROM 地址、Bank、指针或十六进制，会按“想修改什么”列出地图、部署、商店事件、机体、人物与头像、武器、剧情、字库、地图动画、存档等任务；选中任务后点“打开这个功能”即可直达对应页。尚未打开 ROM 时，仅开放“选择 ROM”和不依赖 ROM 的存档修改器，其余入口会明确显示“请先打开 ROM”。
...
第一次使用只需记住五步：打开推荐 ROM → 按 `F1` 选择任务 → 在页面内选择对象并修改 → 点页面“暂存/应用”及独立窗口“确定” → 回到主窗口按 `Ctrl+S` 另存派生 ROM。这里有三级状态：页面“暂存/应用”只把当前表单交给独立窗口；独立窗口“确定”才交给主程序；`Ctrl+S` 才真正写出 `.nes` 文件。页面按钮因“没有变化”而变灰是正常状态；因容量或证据不足而受保护的入口会显示原因和安全替代路径。
...
- 数据库人物页按参考修改器恢复为单页同屏布局：头像设置在上方，名称、音乐、人物属性与精神在左侧，攻击/防御/变形台词在右侧；不再拆成“基本信息与头像、属性与精神、战斗台词”三个产品自创页签。左侧人物列表和参考版“添加”入口保持可见；当前固定池没有新增人物容量时，点击“添加”只显示容量说明，不写 ROM。约 1220×787 逻辑像素下不产生横向或纵向滚动。
- 同一轮全界面审计还校正了数据库机体与武器页：机体的基本设置、属性和两件武器在真实中文字体下仍保持同屏；武器的名称、六字节战斗参数、特技/距离补正改为紧凑上下区，双方动画紧随其后，功能范围和 ROM 原始记录折叠保留。在约 1220×787 逻辑像素下，机体、人物、武器三页详情均不产生横向或纵向滚动。战斗属性计算器继续保持敌我左右对照，存档修改器继续保持上下两张表，“其他”继续保持公式区与初始机体左右分区；三者不再被长下拉内容或固定尺寸强制撑出屏幕，可在不改变旧版操作顺序的前提下自由缩放。
...
- 三类数据库均可直接输入新名称。名称按各自共享池总容量重排并同步指针；总量超过已验证池尾时才会拒绝。名称记录被多个 ID 共用时会列出全部受影响 ID 并要求确认。若还要切换“名称引用”，请分两次应用，避免误改新引用指向的共用文字。
...
- 机体拼图与碎片拼图现重新按两份一手参考窗口拆分。机体拼图底部不再显示额外的“上传机体/导出 BMP/清除机体”按钮，导入、导出、复制、粘贴、删除及清空仍从图库右键菜单进入；顶部保留清除、四种模板和查看效果。碎片拼图不再是单纯的原始图库浏览页：顶部恢复机体类型、碎片/主体图库地址、碎片原码、清除、查看效果及水平/垂直整体翻转；左侧并排显示两个 1 KiB 碎片图库并列出每项 X/Y、图块号和翻转标志，右侧用独立 `128×128` 效果图和四向按钮还原参考操作。效果图左键打开单项位置/图块/翻转编辑，右键切换命中项的水平翻转；所有修改仍先进入草稿，子窗口确定后才纳入数据库事务。
- 图标绑定和机体大小类型转换已经开放；仍不支持任意拆分或追加图形空间。主体与碎片脚本编辑只在已经完成机体扩容绑定的工程中开放，以便安全重排变长记录。属性记录内的变形关系编码已经开放，但它不等于改写战斗图库大小类型。
- 人物页列出完整的 `$01—$C8` 人物槽，分别编辑目录名称和战斗名称，并开放双方战斗音乐、精神值与成长、五项修正、24 种精神及全局消耗、“击落不消失”和头像设置。两套名称指针共用 395 字节固定池，保存时会去重重排并保留其他人物文字；容量不足会在写入前拒绝并保留表单草稿。当前名称、属性和头像三个固定池都没有新增人物所需的完整容量，因此主界面不显示无效“添加”按钮；需要替换人物时直接选择现有槽位修改或复制。`$C8` 是只有目录名称、属性和头像的保留槽，没有战斗名称或音乐选择器。头像页只显示一张 64×64 合成预览，“显示正面/显示背景”可独立开关两层；可编辑图库、位置和配色，并上传 32×32 图像。属性与头像记录可能共用；勾选“同时修改共用属性记录/头像记录”后，应用时会再次列出全部受影响人物并默认取消；头像上传会按实际 CHR 图块列出所有正面或背景使用者。精神菜单最多显示按顺序排列的前 6 项，消耗值对全部人物生效。
...
- 人物页按参考版提供直接可读的头像和台词操作：正面/背景图库显示为“图库: 003”一类下拉项，位置显示为“头像1—4”，三枚颜色是可点击的真实 NES 色块并展开完整 64 色板；精神列表把消耗显示在名称旁，双击任一精神会打开“精神修改”弹窗。战斗台词的攻击命中/受阻和六种防御结果直接显示实际正文预览，点击后按文字组与正文选择，不要求初学者手工输入十六进制编号。
- 人物页的“战斗台词绑定”还包含一次/二次/三次特殊攻击规则和变形起飞绑定。特殊规则可新增、删除、修改条件并通过正文选择器修改文字段和编号；条件单元格同时提示相同编号对应的人物/机体或武器/机体名称。规则长度变化时会在已验证的人物台词共享池中原子重排全部指针与记录，保持原有共用关系；容量不足会整次拒绝并保留草稿。共用同一台词记录时，提交会列出全部受影响人物并默认取消。变形绑定可添加、清空并限定机体范围，正文对应“战斗对话”页的 `05 · 防御特殊对话`；固定表最多 15 项，满容量时须先清空旧项。
- 武器页列出完整的 `$01—$FF` 武器槽，开放名称、特技、射程、命中、距离修正方式和三种地形攻击值，并用“我方武器动画 / 敌方武器动画 / 使用此武器的机体”三个子页签显示双方实际动画脚本和反向引用。属性指针旁会列出共用同一六字节记录的武器 ID；直接修改、复制、还原或编辑共享动画参数时，会列出全部受影响武器并默认取消。“代码编辑”和指令右键菜单支持对当前单武器动画插入、删除及完整脚本结构编辑；右键“编辑”只打开当前指令参数，右键“插入”先打开不含当前参数的指令类型目录，选定新指令后才进入新指令参数页。超过原指针槽时会移动后续脚本并更新指针，`FE` 的内部与跨记录绝对循环目标同步重定位，被引用指令被删或目标无法证明时拒绝保存。`F8/F9` 是创建对象命令及资源参数，不按地址搬移。“规律”打开武器专用的光束组图、两套物理运行和物理图片四个规律库，未验证规律原码保持只读；“动画测试”生成独立测试 ROM 和存档，不覆盖原 ROM。当前 `$01—$FF` 已满，“添加”只说明武器记录容量，不伪造新武器槽。
...
- “其他修改1”严格按参考修改器保持同屏四列：系统文字、成长方式、升级经验、武器距离命中补正，不拆成产品自创的任务页签。经验表显示等级 1—99 的升级阈值、当前累计和还需经验；可编辑升级阈值以及 4×16 武器距离命中补正，后一级累计经验低于前一级时会拒绝提交。成长表按编号 201—253 显示每级 0—15 的数值，“成长修改(16进制快捷修改)”可一次编辑前 60 级的 60 位十六进制串，等级 61—99 保持原值。214—253 在当前 ROM 共用一条记录，修改会同步影响共用编号；低于 201 的固定成长值在人物/机体属性中设置。当前受支持 ROM 的运行时比较指令、99 项累计经验和每条 50 字节成长记录共同确认等级上限已是 99，界面会显示真实值。旧版 3778 字节的 60→61 样本来自当前产品不支持的 `audit.nes`，不能套用；“更改等级上限”可点击查看 99 级容量诊断，但超过 99 级的未验证重排不会写入。系统文字“添加”同样会解释固定指针表/文字池边界并保持零写入。可运行 `.\.venv\Scripts\python.exe tools\report_m09_other1_compatibility.py` 统一复验 4×16 命中、99 项经验、221 条系统文字、53 个成长槽、单半字节/60 位快捷编辑、214—253 共享和 99 级兼容性；报告位于 `output/verification/m09-other1-compatibility.json`，其中嵌入原 `m09-level-cap-compatibility` 结论。
...
剧情事件窗口包含“关卡设置、行动事件、劝降事件、地图事件、剧情对话、胜利文字”六页，支持缩放。只有关卡设置和地图事件显示章节侧栏；行动与劝降为全局表，不跟随当前章节误筛选。“关卡设置”内还有“界面事件、回合事件、即时事件”三个子页。三个子页只显示参考式自然语言列表；右键“编辑”只打开当前操作码的专用参数窗口，不显示命令目录，也不能误换指令类型。右键“插入（接上/接下）”只打开旧版三页“事件指令”目录，窗口中不显示当前指令参数；单击目录中的命令后，才继续打开该命令自己的参数窗口，确定后完成插入。三页内容严格按旧版划分：第 1 页为触发条件，第 2 页为执行事件 1—4，第 3 页为执行事件 5—7及事件结束；按钮分组、合并命令和文案沿用旧版。右键菜单按参考顺序显示插入、添加增援、编辑、代码编辑、剪切、复制/全部、粘贴/全部、删除和清空；单条编辑、结构插入/删除、剪切和粘贴均接入当前章节事务。人物、头像、机体、道具、音乐、剧情文字组、事件开关、移动限制和状态位等具名参数使用下拉框并显示名称或位含义；坐标、数量、等级、HP、金钱、帧数和地址等真正数值才使用数字框，鼠标停留可查看原始十六进制值。事件列表不再拼接难懂的字段名和十六进制原码，而按旧版显示“判断：……？”、“是/否：转：…”、“文字显示：内容…”、“播放我方/敌方地图音乐…”等可直接阅读的结果；原码仍由独立“代码编辑”入口查看。独立行动表也只在主页面显示自然语言指令列表，不再内嵌当前原码和重复操作按钮；全部编辑、插入及代码操作从右键菜单进入，其中“插入（接上/接下）”使用同一两段式流程。劝降、地图、剧情对话和胜利文字列表支持右键打开相应编辑器。

- 关卡设置从真实的分 Bank 指针读取三类事件，显示当前关卡的完整指令列表、文本摘要和原码。可修改参数或整条合法指令，也可插入、删除；保存时在对应 Bank 内重排并维护入口和绝对跳转。第一关的初始配置表为空，出击通过界面事件安排；地图上的空配置列表并非遗漏了事件部署。
...
- 行动指令及三类关卡事件均可通过右键复制、编辑、代码编辑、接上/接下插入、删除与清空。Bank `$26` 独立行动表会重排 256 项对象池；界面/回合/即时事件分别在 `$1E/$1B/$1F` 固定池内移动后续记录并更新三张入口表。两条路径都会重定位 `$55/$57/$58` 绝对跳转；容量不足、删除被引用指令、目标悬空或命中 `$1B:$BE9B` 等重叠解释区时会在写入前整体拒绝。
- 劝降页只开放 ROM 中 4 条已确认规则；其余占位槽不会伪装成可用事件。
- 剧情文本支持 Unicode 与原始 Token 无损往返。当前开放 `$32/$33/$36/$37/$38/$39/$3A/$3B` 八组共 1837 个索引；未知控制参数和未改字形的 Token 别名都按原始字节保留。`$32/$33/$36/$38/$39/$3A/$3B` 即使尚未建立扩展计划，也会在各自已验证的原 Bank 安全池内自动重排指针和正文；总池不足时才需要缩短同组文字或使用容量规划。`$39` 严格止于 `$887E`，不会覆盖相邻运行数据。`$37` 是分离指针/数据布局：`$00` 指向非文本结构并固定只读，`$01—$33` 共 51 个逻辑槽对应 34 条物理文本记录，会在 `$9B4A—$9E27` 的 734 字节固定池内整体重排并保持别名。基准池已满，因此增长一条前必须先缩短同组另一条。
...
- 标题拼图预览已改为直接解析 ROM 真实图块：32 项指针表在文件 `0x16111`，每关三个 64 图块 CHR 页表在 `0x159AE`，不再用关卡列表文字模拟。因此第 1 关黑框显示的是 ROM 内实际“克鲁泽之影”，而不是列表占位名“伏击之战”。根据用户提供的原版/产品并排截图，标题像素索引 1/2/3 也应为亮→暗 `$20/$10/$00`，即 `#FCFCFC/#BCBCBC/#747474`：白色是主笔画，深灰是边缘。标题拼图窗口按参考版恢复为顶部坐标/宽度和刷新按钮、中部黑底标题预览、下部三块图库横排；原始 `FE X Y 宽度 + 两行图块 + FF` 脚本移入“查看拼图代码”弹窗，避免普通界面拥挤。当前只允许保持原记录字节数的原位写入，指针表不变。
...
`.dcfontset` v1 包含 12 页共 2688 个独立字模、48384 字节点阵和 UTF-8 自定义映射，导入会严格检查魔数、版本、长度、代码和值，并在完整校验后一次暂存；不包含 E/F 别名或 192 个行保留区。“写入文字”在点阵尚未变化时会保持灰色，这是避免无变化提交的状态，不是功能未开放；在 12×12 点阵中左键绘制或右键擦除后按钮立即启用。右键增强不增加参考版可见控件。2026-09-19 实际复核参考窗口未发现自动编码或文件导入/导出入口，因此该格式是新修改器的版本化产品协议，不宣称兼容未知的参考文件格式。可运行 `.\.venv\Scripts\python.exe tools\report_m11_font_compatibility.py` 复核，报告位于 `output/verification/m11-font-compatibility.json`。确定后仍需保存工程/派生 ROM；其他未核验 ROM 布局只读。

![地图动画](images/dc_modifier/05-map-animation.png)

地图动画包含“动画、规律、调用”三个分类，直接读取当前 ROM。动画页显示真实指令、地址、共享编号和原始代码；选中指令后可修改已验证参数，并通过右键或代码区插入、删除完整指令。超出当前分配时会搬移同表后续脚本、更新指针，并同步重定位当前脚本内部、其他脚本跳入当前脚本及后续脚本中的 `FE` 绝对循环目标；被引用目标被删除或无法唯一识别时整项拒绝。“添加”会把当前动画复制到下一个 `$3F—$98` 预留槽，并把记录内循环地址重定位到副本。参考按钮“代码编辑”仍按旧版动线打开“请输入动画指针”对话框；确认当前表中的既有指针后定位并展开结构代码区。不存在的指针会明确拒绝且不修改 ROM；任意指针表改绑到未知地址的协议仍不开放。

规律页读取背景、运行和组图记录，并按参考版操作方式分成“背景规律 / 运行规律 / 组图规律”三个分类，一次只显示当前规律族，不再把三套完整编辑器横向挤在同一页。已确认解释方式的运行规律可改帧、位移、音效和循环次数。组图前两字节按当前 ROM 渲染例程确认为有符号 X/Y 起点，但参考 EXE 现场采集证明这两个界面值在“确定→保存”后不会写入 ROM，冷启动仍回到原值，因此新修改器只读展示 X/Y，不提供微调按钮。组图代码当前只开放已有保存/不同 PID 重开黄金的首图块字节 `$00—$EF`；后续拼图命令、运行时令牌与记录边界保持原值。背景规律的 106 个指针槽并非统一定长记录；现已按 ROM 内 `$D3E8` 取指、`$D3F7` 分发表和 `$D4FD—$D8D9` 处理器逐项分类：75 条能完整到达唯一结束边界，其中 69 条含可编辑参数，只允许修改 `$00—$ED` 绘制数据及 `$F8/$F9` 的两个固定参数。其余 6 条完整记录没有安全参数，31 条含动态地址、运行时长度、指针换页、提前结束尾部或缺失结束边界，保持只读。操作码、资源调用、变长/运行时参数、`FE` 布局头、`FF` 结束码和指针始终锁定；即使保持总长度，也不能把控制码当普通正文改写。地图动画和三类规律的名称框可编辑；名称保存在 `.dcmod` 工程元数据中，不修改 ROM 字节或全局默认配置，只打开派生 ROM 时需同时打开对应工程才能恢复自定义名称。运行规律“添加”只使用 `$7D—$9C` 的 32 字节 `FF` 尾池：当前地图动画必须恰好一次引用所选规律，且该位置的“组图帧序列/坐标位移”角色与源项一致；复制和把该唯一引用改到副本作为一个事务执行。首次最多复制 31 字节，未引用、多次引用、角色冲突或容量不足都会拒绝。组图规律“添加”只使用未被运行规律引用的 `$EB—$F6` 可调用尾槽，在 `$B338—$B38F` 的 88 字节尾池内从后向前分配，并把 `$F7/$F8` 保留为控制操作码；保留 5 字节空组图后，首次最多复制 83 字节。调用页列出 86 个实际 `38 02 id` 位置，其中 78 个已由完整上下文、资料集地址/编号双证据或参考逐字段同址保存黄金核对，可更换已有动画编号；其余 8 个位置保持禁用和只读。M12 报告已为这 8 处逐条记录文件偏移、Bank/CPU 地址、邻近原码和锁定原因：`$3804A` 的参考编号与当前 ROM 冲突，`$38982` 仅对应到资料集疑似偏一字节的 `$38983`，其余位置没有地址与完整调用上下文的双重证据，均不按推测解禁。

已核对的调用下拉框可在同一窗口内连续更换编号，也可改回打开窗口时的原编号；“确定”后保存派生 ROM 并重新打开应保留最后选择。地址/原编号或同址保存黄金以打开窗口时的 ROM 指令边界为准，后续每次仍检查当前调用指令和目标动画是否安全；这不会解禁其余 8 个只读调用。

为了让初学者能判断“为什么不能改”，背景选择后会直接显示可编辑参数数量，或动态地址、运行时长度、指针换页、尾部/结束码不完整等具体只读原因及相对偏移。调用表增加“状态/原因”列：78 个可写位置显示证据类型，8 个只读位置逐项显示编号冲突、地址偏差、仅有编号语义或嵌入数据未证等原因；禁用下拉框的提示使用同一说明，不再只显示笼统禁用。

背景/运行/组图规律仍按各自已验证参数门禁；非法动态代码会阻止确定并保留输入。地图及敌我动画主脚本允许完整指令级结构编辑，并自动维护已证明的 `FE` 控制流。共享记录的改动会影响全部引用者。地图动画窗口使用本地草稿，动画副本、代码、调用和工程名称在“确定”时合并为一次撤销事务，“取消”不修改工程。

![动画物理拼图](images/dc_modifier/05b-animation-puzzle.png)

组图页的“动画拼图”会打开“物理拼图”：可切换真实 4 KiB CHR 预览窗口，调整 `00/80` 两段逻辑图块映射，查看 256 块图库、逐条图块地址/X/Y/调色板/水平或垂直翻转，并显示实际拼图。窗口还可选择已确认是“组图帧序列”的运行规律，逐帧、连续或循环播放；这些预览控件不写 ROM。当前 249/249 个组图记录均可完整解释，81 个帧规律中 79 个可离线完整播放；规律 `$12` 的循环次数来自运行时参数，`$21` 会切换运行时指针页，因此两项明确提示而不猜测。13 个组图使用 `$F0—$FF` 运行时图块令牌，对应块显示为未解析占位。运行/组图复制会连同引用或名称作为同一窗口事务提交并可撤销；指令体通用写回、任意通用搬移、37 条无安全参数或动态/不完整背景及其余 8 个调用仍未开放。

可运行 `.\.venv\Scripts\python.exe tools\report_m12_animation_compatibility.py` 复验六张指针表、地图/运行/组图/背景/调用的单字段差分、地图动画/运行规律/组图三类预留槽复制、非法写入拒绝、249 个物理拼图、离线播放及 8 个只读调用的逐项证据审计；报告写入 `output/verification/m12-animation-compatibility.json`。参考窗口三页签、组图首图块正向黄金与 X/Y 否定黄金汇总在 `output/verification/m12-reference-dynamic-2026-09-19.json`。报告通过代表当前安全实现范围完成，但仍不等于用户签收，其他逐字段参考黄金仍可继续补强。
...
“开始计算”会分别输出我方和敌方的命中临界、最低命中速度、严格大于判定的双击边界、预计伤害、机体防御特技后实际伤害、剩余 HP 和击落次数。命中使用当前武器距离补正表的最大射程列后与 M17 临界值比较；伤害按目标机体的空/陆/海类型选用对应火力。本窗口始终不写 ROM，无确定/取消。参考版两个“更改倍数”按钮会触发运行时错误；新修改器不复刻崩溃，而是把它们实现为安全的分子/分母编辑框，范围均为 1—99，只影响当前窗口本次计算。倍率也可直接按“分子/分母”格式输入。实现侧机器门禁见 `output/verification/m15-attribute-calculator.json`，完整现场步骤见 `docs/M15验收清单.md`；该增强不宣称参考版按钮已有正常黄金行为。
...
修改器现在按资源池而不是按单条记录判断容量：已确认“指针表＋连续池”或“顺序记录池”的资源，会重排池内记录并同步指针；脚本还会同步已验证的内部跳转。可运行 `\.\.venv\Scripts\python.exe tools\report_legacy_memory_distribution.py`，从旧修改器保存前后 ROM 证据重新生成 `output/verification/legacy-memory-distribution.json` 和 `docs/research/legacy-modifier/旧修改器全模块内存分布.md`。报告里的写入区间只表示旧版实际改过哪些字节，不等于空闲空间；没有增长/缩短、别名和运行时证据的区域仍会保持固定长度或只读。

机体、人物和武器名称均按各自共享池的总容量判断，不再要求每条名称与原槽等长。若名称池已满，可先缩短同池另一名称再增长目标名称；修改器会重排指针并保留原有共用关系。总量超出已验证池尾时整次操作会拒绝，不会借用相邻未知数据。
