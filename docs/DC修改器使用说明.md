...
左下选择关卡；“高级地图数据…”中的“应用地图、部署与事件”用于显式提交当前草稿。切换关卡时，合法草稿会自动提交到本次内存会话；尺寸、坐标、容量或 Token 不合法时会阻止切换，并保留当前表单供修正。保存或输出 ROM 前也会自动尝试提交当前地图草稿。
...
文字库按 16×16 网格显示已确认码表中的字符、字符代码和核对过的 ROM 地址。预览直接解码 ROM 中每字 18 字节的 12×12 点阵，不再用本机字体替代；“写入文字、替换全部字体、清空本页”等高风险操作仍需完成写入差分后才会开放。
