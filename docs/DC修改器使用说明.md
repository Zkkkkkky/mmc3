...
剧情事件窗口包含“关卡设置、行动事件、劝降事件、地图事件、剧情对话、胜利文字”六页，支持缩放。只有关卡设置和地图事件显示章节侧栏；行动与劝降为全局表，不跟随当前章节误筛选。“关卡设置”内还有“界面事件、回合事件、即时事件”三个子页。三个子页在主窗口显示参考式反汇编列表；选中一条后点击“编辑所选事件指令…”即可打开等长参数/原码编辑窗口，也可直接双击列表行，返回后列表会同步刷新。
