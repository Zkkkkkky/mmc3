...
本工具用于编辑扩容后的 FC《第二次机器人大战》新 DC 篇 ROM。3.0 版恢复了参考 SRW2 修改器的窗口和操作顺序：先经过启动器，主窗口固定为战场地图，各类数据从“数据”菜单打开独立窗口；菜单栏首先连续保留旧版的“文件、数据、帮助”，容量规划、机体包、音乐和工程验证等新功能随后集中在“扩展功能”或“工程”菜单。

2026-09-22 已对旧版 11 个主要可视窗口执行截图级黄金审阅。新版仍保留字库点阵编辑、动画原始字节列、地图对象叠加和容量状态等增强，但这些能力放在附加工具条、状态区、右键菜单或新增列中，不替换旧版入口。可运行 `.\.venv\Scripts\python.exe tools\capture_dc_modifier_guide.py --m01-m02-only` 更新主窗口三页截图，再运行 `.\.venv\Scripts\python.exe tools\build_ui_golden_comparison.py` 生成 `output/verification/ui-golden-2026-09-22/` 下的 11 组并排图、总览图、Markdown 和 JSON 清单。Windows XP 窗口边框与当前 Qt 主题不做逐像素相等判定；本门禁核对信息层级、可视内容和操作入口，字段保存及用户签收仍按模块独立验证。
...
初始配置的黄金截图会主动展开三组只读图标库并收起空部署明细，以便直接核对旧版地址行和真实图标；这只是截图状态，日常默认仍为图标库收起、部署明细展开。

![初始配置可视化对照态](images/dc_modifier/01b-initial-config.png)

![商店事件可视化对照态](images/dc_modifier/01c-shop-event.png)

> **M03/M04 当前写入门禁：**参考版直接字段已完成逐字段保存与全新进程回读（M03 安全分母 1380，M04 为 103），但部署类型切换、增删/重排等结构操作仍缺安全写入协议，因此正式界面继续只允许查看列表、图标、地图标记、悬停信息和只读设置弹窗。部署/事件表格编辑、增删、地图拖动、弹窗确定以及对应项目写回均保持禁用；地形绘制和地图尺寸等 M02 功能不受影响。商店选择中 F0—F4 和 FF 可选，F5—FE 会显示为无效指针并禁选。直接字段证据通过不自动解除结构写入门禁。
