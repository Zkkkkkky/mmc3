...
关卡切换会复用部署下拉数据、表格单元格编辑器、地形图块、机体小图标、人物名称指针容量、成长表、说明文字和事件池解析结果；折叠的机体图标库只更新当前关卡地址，展开时才生成整页预览。基准 ROM 前12关在本机可见页面连续切换的首次平均耗时约 `0.022 秒/关`、最慢约 `0.075 秒`，已访问关卡往返平均约 `0.009 秒/关`；更换 ROM、名称指针表变化或从其他页面修改相关数据时，对应缓存会自动失效并重新读取，不会显示旧工程内容。
