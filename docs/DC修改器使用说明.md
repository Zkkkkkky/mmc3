...
- 数据库人物页也采用参考版的首屏工作区：头像预览、图库/位置/三色以及上传导出位于顶部横栏；名称、战斗名称、双方音乐、人物属性和精神位于左侧；八条直接战斗台词位于右侧，按攻击与防御顺序纵向排列。功能范围、指针、共享记录和原始字节保留在可展开的说明中，不再挤占日常编辑区。全修改器的页签、分组框、按钮、输入框和表头统一收紧间距；在约 1220×787 逻辑像素下，人物核心字段可在同一屏查看且不产生横向或纵向滚动。
...
- 人物页列出完整的 `$01—$C8` 人物槽，分别编辑目录名称和战斗名称，并开放双方战斗音乐、精神值与成长、五项修正、24 种精神及全局消耗、“击落不消失”和头像设置。两套名称指针共用 395 字节固定池，保存时会去重重排并保留其他人物文字；容量不足会在写入前拒绝并保留表单草稿。人物列表下方保留参考版“添加”入口；当前名称、属性和头像三个固定池都没有剩余容量，点击会显示精确容量说明并保持 ROM 零写入，不再只是无解释的灰按钮。`$C8` 是只有目录名称、属性和头像的保留槽，没有战斗名称或音乐选择器。头像页与原修改器一样只显示一张 64×64 合成预览，“显示正面/显示背景”可独立开关两层；可编辑图库、位置和配色，并上传 32×32 图像。属性与头像记录可能共用；勾选“同时修改共用属性记录/头像记录”后，暂存时会再次列出全部受影响人物并默认取消；头像上传会按实际 CHR 图块列出所有正面或背景使用者。精神菜单最多显示按顺序排列的前 6 项，消耗值对全部人物生效。
...
- 武器页列出完整的 `$01—$FF` 武器槽，开放名称、特技、射程、命中、距离修正方式和三种地形攻击值，并用“我方武器动画 / 敌方武器动画 / 使用此武器的机体”三个子页签显示双方实际动画脚本和反向引用。属性指针旁会列出共用同一六字节记录的武器 ID；直接修改、复制、还原或编辑共享动画参数时，会列出全部受影响武器并默认取消。“代码编辑”展开当前敌我模式的等长十六进制编辑区；“规律”打开武器专用的光束组图、两套物理运行和物理图片四个规律库，规律名称可编辑并随 `.dcmod` 工程保存、取消、撤销和重开，规律代码在取得逐字段黄金前保持只读；“动画测试”会先暂存当前表单，再生成 `output/verification/weapon-animation-XX.nes` 及配套活动战场存档，只在该测试 ROM 中把当前武器装到测试机体并启动随附 Mesen。标题画面选择 `CONTINUE`，选中测试机体后可直接用该武器攻击相邻敌军；原 ROM、原存档和当前工程不会被覆盖。当前 `$01—$FF` 已满，“添加”可点击查看容量说明但不会生成新武器；动画脚本继续采用既有记录的等长参数编辑。
...
剧情事件窗口包含“关卡设置、行动事件、劝降事件、地图事件、剧情对话、胜利文字”六页，支持缩放。只有关卡设置和地图事件显示章节侧栏；行动与劝降为全局表，不跟随当前章节误筛选。“关卡设置”内还有“界面事件、回合事件、即时事件”三个子页。三个子页只显示参考式反汇编列表；在一行上单击鼠标右键，再选“编辑”，才会弹出参考式三页事件指令窗口，不再把大型编辑器直接挤在主窗口中。右键菜单按参考顺序显示插入、添加增援、编辑、代码编辑、剪切、复制/全部、粘贴/全部、删除和清空；其中编辑、代码编辑及等长复制粘贴已接入当前章节事务，尚无章节脚本安全重排证据的结构操作会保留原位置并明确置灰。指令按钮和当前参数区同时显示中文名称、用途说明和具名参数；与当前槽位长度不兼容的命令会置灰并提示所需字节数。专家仍可点“代码编辑…”修改一条完整十六进制指令。劝降、地图、剧情对话和胜利文字列表也支持右键打开相应编辑器。
...
- 行动指令可复制、粘贴为草稿、编辑，也可用“插入（接上）/插入（接下）”在当前指令前后新增一条。上述操作可使用可见按钮，也可在指令列表右键选择；行动列表右键可还原当前行动。右键会先切换到鼠标所在行，再调用与按钮完全相同的事务逻辑。变长写入会原子重排整个行动池、更新 256 项指针，并重定位 `$55/$57/$58` 的池内绝对跳转；重复的单字节 `DF` 空行动只占一份物理空间。超出 2751 字节、删除后使跳转悬空、使记录越界或留下空记录时会在写入前整体拒绝。这项变长能力只对独立行动表开放，关卡设置和地图事件仍保持等长。
- 劝降页只开放 ROM 中 4 条已确认规则；其余占位槽不会伪装成可用事件。
- 剧情文本支持 Unicode 与原始 Token 无损往返。当前开放 `$32/$33/$36/$37/$38/$39/$3A/$3B` 八组共 1837 个索引；未知控制参数和未改字形的 Token 别名都按原始字节保留。`$32/$33/$36/$38/$39/$3A/$3B` 即使尚未建立扩展计划，也会在各自已验证的原 Bank 安全池内自动重排指针和正文；总池不足时才需要缩短同组文字或使用容量规划。`$39` 严格止于 `$887E`，不会覆盖相邻运行数据。`$37` 是分离指针/数据布局：`$00` 指向非文本结构并固定只读，`$01—$33` 共 51 个文本槽支持等长原位编辑；该组不自动搬移。
...
地图动画包含“动画、规律、调用”三个分类，直接读取当前 ROM。动画页显示真实指令、地址、共享编号和原始代码；选中指令后可修改已验证的等待、配色、图库、坐标、音乐/音效及循环次数，右键指令表可显示等长原码编辑区。“添加”会把当前动画复制到下一个 `$3F—$98` 预留槽，并把记录内循环地址重定位到副本；写入只使用当前表内已验证的 2306 字节全零池，空间或槽位不足时整项拒绝。参考按钮“代码编辑”仍按旧版动线打开“请输入动画指针”对话框，并自动带入当前指针的低字节在前表示；确认一个当前表中的既有指针后会定位到对应动画并展开等长代码区。不存在的指针会明确拒绝且不修改 ROM；任意指针表写回协议尚无黄金差分，因此该入口只负责安全定位，真正保存仍只允许已有指令中的已验证参数。
