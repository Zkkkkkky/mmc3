...
地图动画包含“动画、规律、调用”三个分类，直接读取当前 ROM。动画页显示真实指令、地址、共享编号和原始代码；选中指令后可修改已验证的等待、配色、图库、坐标、音乐/音效及循环次数，右键指令表可显示等长原码编辑区。参考按钮“代码编辑”现按旧版动线打开“请输入动画指针”对话框；指针写回协议尚无黄金差分，确认输入只显示门禁说明，不修改 ROM。规律页读取背景、运行和组图记录：已确认解释方式的运行规律可改帧、位移、音效和循环次数，组图可改起始坐标，背景规律暂只读。调用页列出实际调用位置，只有已核对完整指令序列的调用可更换已有动画编号。

原记录的操作码、可变长度字段、资源引用和跳转目标保持不变；非法或变长代码会阻止确定并保留输入。共享记录的改动会影响全部引用者。地图动画窗口使用本地草稿，“确定”合并为一次撤销事务，“取消”不修改工程。组图页的“动画拼图”已接通标题为“物理拼图”的只读次级窗口，显示当前组图原码与效果网格；图块组装/翻转写回、新增/搬移脚本、动画播放测试及全部调用的精神/地图武器名称关联仍未开放。
